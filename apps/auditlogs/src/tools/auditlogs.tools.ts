...
export const auditLogsQuerySchema = z.object({
	...
	"account_name.not": z.array(z.string()).optional().describe('Filter to audit logs matching the provided account names.'),
	"action_result.not": z.array(actionResults).optional().describe('Filter to audit logs matching the provided action results (success or failure).'),
	"action_type.not": z.array(actionTypes).optional().describe('Filter to audit logs matching the provided action types.'),
	"actor_context.not": z.array(actorContexts).optional().describe('Filter to audit logs matching the provided actor contexts.'),
	"actor_email.not": z.array(z.string().email())
		.optional()
		.describe('Filter to audit logs from users with the provided emails.'),
	"actor_id.not": z.array(z.string()).optional().describe('Filter to audit logs matching the provided unique identifiers.'),
	"actor_ip_address.not": z.array(z.string()).optional().describe('Filter to audit logs from actors with the provided IP addresses.'),
	"actor_token_id.not": z.array(z.string()).optional().describe('Filter to audit logs for actions taken by the provided tokens.'),
	"actor_token_name.not": z.array(z.string()).optional().describe('Filter to audit logs for actions taken by the provided token names.'),
	"actor_type.not": z.array(actorTypes).optional().describe('Filter to audit logs matching the provided actor type.'),
	"audit_log_id.not": z.array(z.string()).optional().describe('Filter to audit logs matching the provided ids.'),
	...
	"raw_method.not": z.array(z.string())
		.optional()
		.describe('Filter to audit logs matching the provided HTTP methods.'),
	"raw_status_code.not": z.array(z.number()).optional().describe('Filter to audit logs matching the provided HTTP status codes.'),
	"raw_uri.not": z.array(z.string()).optional().describe('Filter to audit logs matching the provided URIs.'),
	"resource_id.not": z.array(z.string()).optional().describe('Filter to audit logs matching the provided resource IDs.'),
	"resource_product.not": z.array(z.string())
		.optional()
		.describe('Filter to audit logs matching the provided products.'),
	"resource_type.not": z.array(z.string()).optional().describe('Filter to audit logs matching the provided resource types.'),
	"resource_scope.not": z.array(resourceScopes)
		.optional()
		.describe('Filter to audit logs matching the provided resource scopes.'),
	"zone_id.not": z.array(z.string()).optional().describe('Filter to audit logs matching the provided zone ids.'),
	"zone_name.not": z.array(z.string()).optional().describe('Filter to audit logs matching the provided zone names.'),
	since: z
		.string()
		.describe(
